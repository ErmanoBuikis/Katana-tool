...
def choose_indexes_without_reintroduction(indexes_name_list):
    choosen=rnd.choice(indexes_name_list)
    indexes_name_list.remove(choosen)
...
@jit(nopython = True)
def EMA(arr, period = 5, alpha = False):
    if (alpha == True):
        alpha = 1 / period
    elif (alpha == False):
        alpha = 2 / (period + 1)
